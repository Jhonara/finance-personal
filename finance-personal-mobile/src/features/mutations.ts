...
const invalidateFinancial = (client: ReturnType<typeof useQueryClient>, accounts = true) =>
  Promise.all([
...
export const useOpeningBalanceMutation = () => {
  const client = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: Parameters<typeof createOpeningBalance>[1] }) =>
      createOpeningBalance(id, data),
    retry: false,
    onSuccess: () => invalidateFinancial(client),
  });
};
